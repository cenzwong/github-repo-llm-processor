...
1. Initialize dependencies (`fastapi`, `uvicorn`, `httpx`, `pydantic`, `openai`).
2. Implement the GitHub HTTP fetching logic (Branch -> Tree -> Raw Contents).
3. Implement the string filtering and truncation logic.
4. Wire up the FastAPI `POST /summarize` endpoint and test with the `psf/requests` repository.
